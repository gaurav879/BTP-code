...
from transformers import DistilBertConfig, DistilBertTokenizer, get_linear_schedule_with_warmup, DistilBertModel
...
class IHDModel(torch.nn.Module):
    def __init__(self):
        ...
        self.distil_bert = DistilBertModel.from_pretrained(args.bert_model_path)
        # Average last 4 layers
        # self.pooler = WeightedLayerPooling(
        #     num_hidden_layers=self.distil_bert.config.num_hidden_layers, layer_start=9)
...
def initialize_mixout(model):
    if args.mixout_rate > 0:
        ...
        for name, module in tuple(model.named_modules()):
            if name and 'distil_bert' in name:
...
def main():
    ...
    loss_history.to_csv(os.path.join(
        args.output_dir, 'loss_history_1.csv'), index=False)
