...
class Mixout(InplaceFunction):
    ...
    def forward(cls,
        ...
        if p < 0 or p > 1:
            raise ValueError(
                f"A mix probability of mixout has to be between 0 and 1,  but got {p}")
        ...
        if len(ctx.noise.size()) == 1:
            ...
            ctx.noise = ctx.noise[0].repeat(
                input.size()[0], *([1] * (len(input.size())-1)))
...
class MixLinear(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if self.p < 0 or self.p > 1:
            raise ValueError(
                f"A mix probability of mixout has to be between 0 and 1,  but got {self.p}")
